...
\begin{definition}[Marks]
    A mark is a basic graphical element, a geometric primitive. They are classified according to the number of spatial dimensions they require: 0-dim (points), 1-dim (lines, segments), 2-dim (areas), 3-dim (volumes)
\end{definition}
...
Channels can be divided into 2 different categories, according to the two different sensor modalities on the human perceptual system: identity and magnitude channels. \textbf{Identity} channels give information about what something is or where it is, like \textit{shape, color}. \textbf{Magnitude} channels tell us how much of something there is, the \textit{length, area, saturation, size, ...}
...
\begin{theorem}[The Expressiveness Principle]
...
\begin{theorem}[The Effectiveness Principle]
...
The fig~\ref{fig:3.1} shows one proposed channel ranking. As the figure shows, in both classifications, spatial channels appear at the top of the lists. The choice of which attributes to encode with position is the most central choice in visual encoding, as they will dominate a user's mental model.
...
The fig~\ref{fig:3.2} shows a visual representation of causes of death used in a debate. Where, each rectangle represents a death cause, the size of the rectangle represent years of life lost, the color represents the group of death, and the color intensity represents the change from one year to another.\par
